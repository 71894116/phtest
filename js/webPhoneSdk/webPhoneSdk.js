...
var m7webPhoneUtils = {
    qhbUrl: 'https://webphone.7moor.com',
...
var m7WebPhone = {
    ...
    loginQhbWebPhone: function (loginData) {
        ...
        m7webPhoneUtils.sendPostCommand(action, function (response) {
            ...
            if (!response.Succeed) { // 长时间断网，会带着旧kickToken导致登录jauns失败。
              return
            }
                            ...
                            janus.attach(
                                {
                                    ...
                                    error: function (error) {
                                        ...
                                        var errorData = {eventJson: "newJanusError", catchRecord: error, errorTime: currentTime}
                                    ...
                                    onmessage: function (msg, jsep) {
                                        if (msg && msg['result'] && msg['result']['event'] && (msg['result']['event'] === 'registering' || msg['result']['event'] === 'registered')) {
                                            console.log('registering or registered')
                                        } else {
                                        if (jsep !== undefined) {
                                            current_jsep = jsep;
                                        }
                                        ...
                                        if (error != null && error != undefined) {
                                            ...
                                            var currentTime = moorCall.recordError.getCurrentTime();
                                            var errorData = {eventJson: "onmessageError", catchRecord: error, errorTime: currentTime}
                                            moorCall.recordError.dataHandle(errorData)
                                            alert(error);
                                            return;
                                        }
                                        ...
                                        if (result !== null && result !== undefined && result["event"] !== undefined && result["event"] !== null) {
                                            ...
                                            if (event === 'registered') {
                                                ...
                                                var currentTime = moorCall.recordError.getCurrentTime();
                                                var errorData = {eventJson: "onmessageRegistered", catchRecord: result, errorTime: currentTime}
                                                moorCall.recordError.dataHandle(errorData)
                                                ...
                                                var currentTime = moorCall.recordError.getCurrentTime();
                                                var errorData = {eventJson: "onmessageIncomingcall", catchRecord: result, errorTime: currentTime}
                                                moorCall.recordError.dataHandle(errorData)
...
initM7WebPhone.prototype.init = function (loginData) {
    if (Janus) {
        started = false
        // 注册电话条之前如果存在会话没销毁的，全部销毁掉，如果只销毁当前的会话，会导致多次切换接听方式之后，再次切换回软电话，导致30S心跳无响应之后，反复注册
        ...
        var currentTime = moorCall.recordError.getCurrentTime();
        var errorData = {eventJson: "initDestroy", errorTime: currentTime}
        moorCall.recordError.dataHandle(errorData)
...
initM7WebPhone.prototype.destroy = function (loginData) {
    if (janus) {
        ...
        janus.destroy()
        var currentTime = moorCall.recordError.getCurrentTime();
        var errorData = {eventJson: "destroy", errorTime: currentTime}
        moorCall.recordError.dataHandle(errorData)
    }
}
    ...
    if (sipcall) {
        sipcall.createOffer({
            ...
            success: function (jsep) {
                ...
                sipcall.send({ "message": body, "jsep": jsep });
                var currentTime = moorCall.recordError.getCurrentTime();
                var errorData = {eventJson: "OutboundCreateOfferAccept", errorTime: currentTime}
                moorCall.recordError.dataHandle(errorData)
            },
            error: function (error) {
                m7WebPhone.onError(error)
                var currentTime = moorCall.recordError.getCurrentTime();
                var errorData = {eventJson: "OutboundCreateOfferAcceptError", catchRecord: error, errorTime: currentTime}
                moorCall.recordError.dataHandle(errorData)
            }
        })
        var currentTime = moorCall.recordError.getCurrentTime();
        var errorData = {eventJson: "OutboundCreateOffer", errorTime: currentTime}
        moorCall.recordError.dataHandle(errorData)
    } else {
        m7WebPhone.onError('初始化失败')
        var currentTime = moorCall.recordError.getCurrentTime();
        var errorData = {eventJson: "OutboundCreateOfferError", errorTime: currentTime}
        moorCall.recordError.dataHandle(errorData)
    }
}
...
function registerUsername (media_server, media_username, media_authuser, media_pwd, media_dispalyname) {
    ...
    var currentTime = moorCall.recordError.getCurrentTime();
    var errorData = {eventJson: "registerUsername", errorTime: currentTime}
    moorCall.recordError.dataHandle(errorData)
}
