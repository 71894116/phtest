...
sevenDeclare("moorCall.Phone", null, {
    ...
    m7InnerRinging: null,
    m7DialoutLink: null,
    m7DialoutTransferLink: null,
    m7ThreeWayCallLink: null,
    m7NormalLink: null,
    m7NormalTransferLink: null,
    m7Abate: null,
    m7ListenRing: null,
    m7NormalRing: null,
    callObject: {},
...
                    return
  ...
  m7SendAction: function(action, params, onload, onerror) {
    ...
    hangup: function() {
        var self = this;
        var currentTime = moorCall.recordError.getCurrentTime();
        var errorData = {eventJson: "customHangup", errorTime: currentTime}
        moorCall.recordError.dataHandle(errorData)
...
moorCall.Phone.registerPhoneWsEvent = function (config, WS_PORT, WS_SECURE_PORT) {
    ...
    var heartCheck = {
        ...
        start: function(){
            var self = this;
            if (self.isExecute) {
                self.latestExecutionTime = new Date().getTime();
                return
            } else {
              self.latestExecutionTime = null;
            }
            self.isExecute = true;
            if (this.timeoutObj) {
                window.clearTimeout(this.timeoutObj);
            ...
            this.timeoutObj = window.setTimeout(function(){
                ...
                if (ws.readyState === 1) { // ws正常建立连接时才发ping
                  ...
                  if (self.latestExecutionTime) {
                    let currentTime = new Date().getTime();
                    if ((currentTime - self.latestExecutionTime)/1000 <= 5) {
                      // 发ping途中收到事件时间小于5s 重置完成进行发ping操作
                    ...
                    if (eventData.Response === 'Login') { // 登陆事件
                         if (eventData.SessionID) {
                             ...
                             if (isRelogin) {
                                 ...
                                 account.sessionId = eventData.SessionID
                                 account.accountId = eventData.Account
                                 account.PBX = eventData.PBX
                                 account.url = config.proxy_url
